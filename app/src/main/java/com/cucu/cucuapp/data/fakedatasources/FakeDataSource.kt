...
import com.cucu.cucuapp.data.models.PurchaseState
...
class FakeDataSource @Inject constructor(){
    ...
    Purchase(
        id = "3",
        state = PurchaseState.PendingPayment(),
        products = listOf(
            CartProduct(
    ...
    Purchase(
        id = "2",
        state = PurchaseState.PreparingProducts(),
        products = listOf(
            CartProduct(
                product = Product(4, "Oso teddy", 1900.00, null, 3, "https://media.traveler.es/photos/613767ec652b2e41f8dce491/1:1/w_896,h_896,c_limit/155694.jpg", "Re tierno", 123L, false),
                quantity = 1
    ...
    productsFakeList.forEach {
        list.add(CartProduct(it, (1..9).random()))
    }
    return list
}
